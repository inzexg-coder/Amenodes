...
- [ВЫВОД](#вывод)
- [КАРТА ПРЕОБРАЗОВАНИЙ](#карта-преобразований)
...
Узел поддерживает **5 операций**, выбираемых через выпадающий список в самом узле:

| Ключ операции | Отображаемое имя (EN) | Отображаемое имя (RU) | Формула | Применение |
|---------------|----------------------|----------------------|---------|-------------|
| `div3` | Divide by 3 (Δ/3) | Деление на 3 (Δ/3) | `x / 3` | Систематическая погрешность прибора |
| `div_sqrt12` | Divide by √12 (ω/√12) | Деление на √12 (ω/√12) | `x / √12` | Погрешность округления |
| `sqrt_sum_sq` | √(a² + b²) Pairwise | √(a² + b²) попарно | `√(a² + b²)` | Попарная обработка (неопределённости) |
| `quadratic_sum` | Quadratic Sum √(Σσ²) | Квадратичное сложение √(Σσ²) | `√(σ₁² + σ₂² + …)` | Суммарная погрешность нескольких источников |
| `multiply_by_constant` | Multiply by Constant | Умножение на константу | `x * k` | Доверительный интервал (γ·σ) |

## ПОВЕДЕНИЕ

### Режимы `div3`, `div_sqrt12`, `quadratic_sum`

- Получает входные данные через `graph.getMergedInput(this.id)`.
- Для каждого числового значения применяет соответствующую операцию.
- Результат — массив чисел (для `quadratic_sum` — одно число).
...
3. Для каждой пары извлекает числовые значения из узлов-источников.
4. Вычисляет `√(a² + b²)`.
5. Результат — массив вычисленных значений.

### Режим `multiply_by_constant`

Требует ровно двух входов:
- один вход — массив значений (погрешности)
- второй вход — число (константа, множитель)

Результат — поэлементное произведение массива на константу.

## UI КОМПОНЕНТЫ

CalcNode отображает **кастомный выпадающий список (dropdown)** для выбора операции, стилизованный в космической теме. При выборе новой операции граф автоматически пересчитывается.
...
Обновляет `this.result` и `this.resultStr` в зависимости от выбранной `operation`.

## ОТОБРАЖЕНИЕ

- Заголовок: цветовая схема `calc-header`.
- Отображает:
  - выпадающий список с выбором операции;
  - результат в формате `[0.123456, 0.789012]` или `0.123456`;
...
```javascript
const calcNode = new CalcNode(null, 100, 100, "Погрешности", { operation: "div3" });
graph.addNode(calcNode);
// При входе [9, 12, 15] результат будет [3, 4, 5]

const sqrtNode = new CalcNode(null, 200, 100, "Попарный расчёт", { operation: "sqrt_sum_sq" });
graph.addNode(sqrtNode);
// При входных парах (3, 4) и (5, 12) результат будет [5, 13]

const sumNode = new CalcNode(null, 300, 100, "Суммарная погрешность", { operation: "quadratic_sum" });
// При входе [0.1, 0.2, 0.3] результат будет √(0.01+0.04+0.09) = √0.14 ≈ 0.374165

const ciNode = new CalcNode(null, 400, 100, "Доверительный интервал", { operation: "multiply_by_constant" });
// Входы: массив погрешностей [0.1, 0.2] и константа 1.96 → результат [0.196, 0.392]
```
...
| Направление | Тип | Описание |
|-------------|-----|----------|
| **Входы** | `num`, `array`, `uncert`, `list`, `wlist`, `interval` | Принимает любые числовые данные и неопределённости. |
...
```json
{
  ...
  "title": "Результаты",
  "important": false,
  "originalTitle": "Результаты",
  "rows": [
    { "param": "Value 1", "value": "42.000000" }
  ]
}
```
...
- При переключении режима на `separate` динамически добавляется синий порт для `unmapped` значений.
- Существующие соединения к порту `unmapped` автоматически удаляются при переключении на `passthrough`.
...
```json
{
  ...
  "important": false,
  "originalTitle": "Среднее значение",
  "result": 4,
  "resultStr": "4.000000"
}
```
...
|----------|----------|
| **Тип (`type`)** | `sem` |
| **Ключ локализации** | `nodes.sem` |
| **Иконка** | `fa-solid fa-down-left-and-up-right-to-center` |
| **Автор** | [@Amenoke](https://github.com/inzexg-coder) |
...
| Направление | Тип | Описание |
|-------------|-----|----------|
| **Входы** | `array`, `list`, `wlist`, `num` | Принимает массив чисел. Может быть несколько входов (все значения объединяются). |
...
- **X̄** — выборочное среднее арифметическое
- **n** — объём выборки (количество элементов)
- **Σ** — сумма по всем элементам

## ПОВЕДЕНИЕ

Узел вычисляет стандартную ошибку среднего (Standard Error of Mean, SEM). SEM показывает, насколько выборочное среднее отклоняется от истинного среднего генеральной совокупности. Чем больше выборка, тем меньше SEM.
...
1. Собирает все входящие значения через `graph.getMergedInput(this.id)`.
2. Если количество элементов **меньше 2** – результат `null`, строка `"--"`.
3. Вычисляет среднее арифметическое **X̄**.
4. Вычисляет сумму квадратов отклонений **Σ (Xᵢ - X̄)²**.
5. Вычисляет SEM по формуле **√( SS / (n·(n-1)) )**.
6. Сохраняет результат с 6 знаками после запятой.
...
- Отображает:
  - название узла (локализованное);
  - количество входных соединений;
  - объём выборки (количество элементов);
  - результат в формате `0.123456` или `"--"`.
- При изменении языка интерфейса заголовок и подписи обновляются автоматически.
...
```javascript
const semNode = new SEMNode(null, 100, 100, "SEM расчёт");
graph.addNode(semNode);

// Подключим GroupNode со значениями [2, 4, 4, 4, 5, 5, 7, 9]
// Вычисление:
// n = 8
// среднее = 5
// SS = (2-5)² + (4-5)² + (4-5)² + (4-5)² + (5-5)² + (5-5)² + (7-5)² + (9-5)² = 32
// SEM = √(32 / (8·7)) = √(32 / 56) = √0.5714 ≈ 0.755929

console.log(semNode.getValue()); // [0.755929]
console.log(semNode.resultStr);  // "0.755929"
```

## ПРИМЕЧАНИЯ

- Минимальный размер выборки – **2 элемента**. При меньшем количестве результат отображается как `"--"`.
- SEM всегда неотрицательна.
- Уменьшается с ростом выборки (пропорционально **1/√n**).
- Узел автоматически пересчитывает результат при любом изменении входных данных.
- При нескольких входящих соединениях все значения из всех источников объединяются в одну выборку.

## ТЕОРЕТИЧЕСКАЯ СПРАВКА

Стандартная ошибка среднего (SEM) является мерой точности выборочного среднего как оценки истинного среднего генеральной совокупности.

Свойства:
- SEM = σ / √n (при известном стандартном отклонении σ)
- SEM → 0 при n → ∞
- Используется для построения доверительных интервалов: X̄ ± t·SEM
...
- Убедитесь, что класс узла имеет метод `getMinHeight()`, возвращающий высоту в пикселях. Базовый класс `Node` предоставляет реализацию по умолчанию (80px).
