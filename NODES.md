...
#### 6. Прилипание к сетке (Snap to Grid)

- Включите эту опцию в настройках холста (`Canvas Settings`), чтобы при перетаскивании узлы автоматически выравнивались по линиям сетки.
- Размер сетки и стиль (точки, линии, крестики, отсутствие) также настраиваются в этом диалоге.

#### 7. Панорамирование и масштабирование

- **Правый клик** и перетаскивание — панорамирование рабочей области.
- **Колёсико мыши** — масштабирование.

#### 8. Редактирование заголовка

- **Клик** на заголовок узла — переход в режим редактирования.

#### 9. Отмена/повтор действий
...
console.log(numNode.getValue()); // [3.14]
```
...
console.log(constNode.getValue()); // [3.14159]
```
...
console.log(groupNode.getValue()); // [10, 20]
```
...
| `sqrt_sum_sq` | `calcTypes.sqrt_sum_sq` | √(a² + b²) — попарная обработка входов |
...
Этот режим не использует `getMergedInput`. Вместо этого он самостоятельно обрабатывает входящие рёбра попарно:

1. Получает все входящие рёбра через `graph.getIncomingEdges(this.id)`.
2. Группирует их в пары (0‑1, 2‑3, …).
3. Для каждой пары извлекает числовые значения из узлов-источников через `graph.getSourceValue()`.
4. Если оба значения являются числами, вычисляет `√(a² + b²)` и добавляет в массив результатов.
5. Если пара неполная или значения нечисловые, добавляет `null`.
6. Итоговый массив фильтруется от `null`; результат отображается в виде `[значение1, значение2, …]`.
...
const sqrtNode = new CalcNode(null, 200, 100, "Попарный расчёт", { calcType: "sqrt_sum_sq" });
graph.addNode(sqrtNode);
// При входных парах (3, 4) и (5, 12) результат будет [5, 13]
```
...
console.log(outputNode.rows); // [{ param: "Value 1", value: "42.000000" }]
console.log(outputNode.title); // "Результаты (1 значение)"
```
...
| Направление | Тип | Описание |
|-------------|-----|----------|
| **Входы** | `uncert` / `interval` (один вход – неопределённость), `num` / `array` / `list` / `wlist` (другой вход – множитель) | Ровно **два** входа. **Порядок подключения не важен** – узел автоматически определяет тип каждого входа. |
...
## Что добавить в `Nodes.md` для узла «Среднее арифметическое»

В документацию `Nodes.md` необходимо добавить новый раздел, посвящённый узлу **ArithmeticMeanNode** («Среднее арифметическое»).  
Ниже приведено точное содержимое, которое следует вставить (например, после раздела `ConfidenceIntervalNode` и перед `Руководство по добавлению нового узла`).
...
| Свойство | Значение |
|----------|----------|
| **Тип (`type`)** | `arithmeticMean` |
| **Ключ локализации** | `nodes.arithmeticMean` |
| **Иконка** | `fa-chart-simple` |
| **Автор** | [@Amenoke](https://github.com/inzexg-coder) |
| **Репозиторий** | [![GitHub](https://img.shields.io/badge/Amenodes-181717?style=flat-square&logo=github)](https://github.com/inzexg-coder/Amenodes) |
...
| Направление | Тип | Описание |
|-------------|-----|----------|
| **Входы** | `num`, `array`, `uncert`, `list`, `wlist` | Принимает числовые данные, массивы, неопределённости. Может быть несколько входящих соединений (все значения объединяются). |
| **Выходы** | `num` | Одно число – среднее арифметическое всех входных значений. |

## ПОВЕДЕНИЕ

Узел вычисляет среднее арифметическое всех чисел, поступающих на его входы. Если входных соединений несколько, все значения из всех источников объединяются в один массив, после чего вычисляется среднее.
...
1. Собирает все входящие значения через `graph.getMergedInput(this.id)`.
2. Если входных значений нет – результат `null`, строка `"--"`.
3. Вычисляет сумму всех чисел и делит на их количество.
4. Сохраняет результат в `this.result` (число) и `this.resultStr` (строка с 6 знаками после запятой).

### getValue()

Возвращает `[this.result]` (массив с одним числом).

## ОТОБРАЖЕНИЕ

- Заголовок: стандартный `node-header`.
- Отображает:
  - название узла (локализованное);
  - результат в формате `0.123456`;
  - количество входных соединений.
- При изменении языка интерфейса заголовок и подписи обновляются автоматически.
...
```javascript
const meanNode = new ArithmeticMeanNode(null, 100, 100, "Среднее значение");
graph.addNode(meanNode);

// Подключим три NumberNode со значениями 2, 4, 6
// Результат: (2+4+6)/3 = 4

console.log(meanNode.getValue()); // [4]
console.log(meanNode.resultStr);  // "4.000000"
```

## ЗАМЕЧАНИЯ

- Узел автоматически пересчитывает результат при любом изменении входных данных (добавление/удаление связей, изменение значений источников).
- Если на вход поступает массив (например, от `GroupNode`), все элементы массива участвуют в расчёте среднего наравне с одиночными числами.
- В случае, если среди входных данных встречаются нечисловые значения (например, `null`), они игнорируются (фильтруются в `getMergedInput`).
