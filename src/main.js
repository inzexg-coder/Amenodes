...
class Application {
  ...
  initMiniMap() {
    ...
    if (container) {
      ...
      this.miniMap.onNavigate = (x, y) => {
        this.viewport.setOffset(-x * window.currentZoom, -y * window.currentZoom);
        this.renderer.render();
      };
    }
  }
  ...
  initPanels() {
    ...
    if (closePropertiesBtn && this.propertiesPanel) {
      closePropertiesBtn.onclick = () => this.propertiesPanel.classList.add('hidden');
    }
  }
  ...
  openNodeMenu() {
    if (this.nodeMenu) this.nodeMenu.toggle();
  }
  ...
  applyCanvasSettings() {
    ...
    switch(this.gridStyle) {
      case 'dots':
        ...
        break;
      case 'lines':
        viewport.style.backgroundImage = `linear-gradient(to right, rgba(255, 179, 71, 0.1) 1px, transparent 1px), linear-gradient(to bottom, rgba(255, 179, 71, 0.1) 1px, transparent 1px)`;
        viewport.style.backgroundSize = `${this.gridSize}px ${this.gridSize}px`;
        break;
      case 'cross':
  ...
  initDirtyIndicator() {
    ...
    if (this.graph && typeof this.graph.onDirtyChange === 'function') {
      this.graph.onDirtyChange((isDirty) => {
        ...
        document.title = isDirty ? '* @Amenodes' : '@Amenodes';
      });
    }
  }
  ...
  updateNodeCount() {
    ...
    if (nodeCountEl) nodeCountEl.textContent = this.graph.nodes.length;
  }
  updateEdgeCount() {
    const edgeCountEl = document.getElementById('edgeCount');
    if (edgeCountEl) edgeCountEl.textContent = this.graph.edges.length;
  }
  updateZoomIndicator() {
    const zoomEl = document.getElementById('zoomIndicator');
    if (zoomEl) zoomEl.textContent = `${Math.round((window.currentZoom || 1) * 100)}%`;
  }
  updateCoordIndicator() {
    const offset = this.viewport.getOffset();
    const coordEl = document.getElementById('coordIndicator');
    if (coordEl) coordEl.textContent = `${Math.round(offset.x)}, ${Math.round(offset.y)}`;
  }
  ...
  updatePropertiesPanel(node) {
    ...
    if (!container) return;
    if (!node) {
      container.innerHTML = `<div class="empty-property"><i class="fas fa-hand-pointer"></i><p>${t('editor.selectNodeToEdit')}</p></div>`;
      return;
    }
    container.innerHTML = `
      <div class="property-group"><div class="property-label">${t('editor.nodeType')}</div><div class="property-value">${node.type}</div></div>
      <div class="property-group"><div class="property-label">${t('editor.id')}</div><div class="property-value">${node.id}</div></div>
      <div class="property-group"><div class="property-label">${t('editor.position')}</div><div class="property-value">X: ${Math.round(node.x)}, Y: ${Math.round(node.y)}</div></div>
      <div class="property-group"><div class="property-label">${t('editor.connections')}</div><div class="property-value">${t('editor.inputs')}: ${this.graph.getIncomingEdges(node.id).length}<br>${t('editor.outputs')}: ${this.graph.getOutgoingEdges(node.id).length}</div></div>
    `;
  }
}
