...
> **Примечание:** Ранее при ошибках соединения вызывался `modal.alert()`. Теперь все алерты удалены — визуальная обратная связь (красная рамка на 600мс) происходит в `DomRenderer`.
